...
patched. It then runs a safe post-apply readiness check. A successful confirmed
run must also prove phase-specific OPatch inventory on both patched Data Guard
homes, `DBA_REGISTRY_SQLPATCH` success on the promoted primary after datapatch,
and Maximum Availability with a `READ ONLY WITH APPLY` standby after the final
role state is restored.
